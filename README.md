...
- PTrade 运行脚本盘前读取不到新的订单 JSON 时，会继续保留上一轮有效监控计划；买单依赖 `buy_date` 判断会自然跳过非买入日，止盈/止损单则继续作为持仓风险控制承接。
...
- 不要把 PTrade 运行脚本中“订单 JSON 读取失败时保留上一轮监控计划”改成清空状态；这是有意设计，用于跨日延续止盈/止损监控。
